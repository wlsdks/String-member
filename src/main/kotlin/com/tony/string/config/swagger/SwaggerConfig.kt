...
class SwaggerConfig {
    ...
    fun swaggerApi(): OpenAPI = OpenAPI()
        .components(
            ...
            // 여기부터 추가 부분
            .addSecuritySchemes(
                SECURITY_SCHEME_NAME, SecurityScheme() // addSecuritySchemes()는 인증 정보 입력을 위한 버튼을 추가하는 메서드
                .name(SECURITY_SCHEME_NAME)
                .type(SecurityScheme.Type.HTTP)
                .scheme("bearer")
                .bearerFormat("JWT")))
        .addSecurityItem(SecurityRequirement().addList(SECURITY_SCHEME_NAME)) // addSecurityItem()은 시큐리티 요구 사항을 스웨거에 추가한다.
        // 여기까지
        .info(
            Info()
